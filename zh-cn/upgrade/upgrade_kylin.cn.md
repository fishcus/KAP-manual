...
### 从Apache Kylin 1.5.1+升级至Kyligence Enterprise 2.X###

Kyligence Enterprise 2.X与Kylin 1.5.1+版本兼容元数据。因此在Kylin升级至Kyligence Enterprise时，无需对元数据进行升级，只需要覆盖软件包、更新配置文件并升级HBase协处理器即可。
...
5. 修改配置参数：
   ...
   由于Kyligence Enterprise与Kylin默认的元数据表命名方式不同，如果想要继承Kylin中已有的元数据，需要将`conf/kylin.properties`文件中的参数`kylin.metadata.url`值`kylin_default_instance@hbase`修改为与Kylin一致，默认情况下为`kylin_metadata@hbase` 。

   由于Kyligence Enterprise与Kylin的cube逐层构建算法存在差异，如果想要使用Kylin中已构建的Cube数据，需要将`conf/kylin.properties`文件中的参数`kylin.cube.aggrgroup.is-mandatory-only-valid`的值`true`修改为`false`。
...
### 从Apache Kylin 1.5.1+升级至Kyligence Enterprise

Kyligence Enterprise Plus与Kyligence Enterprise的主要区别在于引入了全新的存储引擎KyStorage。因此在从Kylin升级至Kyligence Enterprise Plus时，只需要按照从Kylin升级至Kyligence Enterprise的方法执行，最后进行存储引擎相关的升级操作即可。具体升级步骤如下：

1. 执行上一节“从Apache Kylin 1.5.1+升级至Kyligence Enterprise 2.X”中的步骤1至8。其中Kyligence Enterprise安装包用Kyligence Enterprise Plus安装包替换。

2. 由于Kyligence Enterprise中新引入的Kyligence Enterprise Query Driver会占用一定系统资源，如果是在沙箱等资源较为紧张的环境下运行Kyligence Enterprise Plus，请调整相关配置以确保Kyligence Enterprise Plus能够获取足够的资源。
...
### 从Apache Kylin 1.5.0及之前版本升级至Kyligence Enterprise 2.X ###
