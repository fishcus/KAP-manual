...
3. KyAnalyzer 依赖于 mondrian 的 jar 包，为了符合其开源协议，需要单独下载并拷贝。
   ...
   * 对于 KyAnalyzer V2.1.3 以上的版本，在Linux环境下，启动脚本会自动下载所需的 mondrian 包并将它拷贝至 tomcat 下
...
#### 对层级维度进行层次化展示
在 KyAnalyzer 中，当您同步 Cube 时，也会将 Cube 中的层级信息同步至 KyAnalyzer。这样您便可以在层级维度执行下钻和上卷操作。![层级维度](images/hierarchy_table.png)
...
隐藏父节点的按钮在运行按钮旁边，默认为开启状态，此时父节点的指标值不单独显示，仅能查看子节点的指标值，关闭该功能之后可以显示父节点的指标值。

如下图所示，可以看到父节点所对应的指标值并没有显示：

![隐藏父节点按钮位置](images/father_node_2.png)

将隐藏父节点取消选择，再执行查询，可以看到显示出父节点的指标值：

![隐藏父节点按钮功能](images/father_node_4.png)

#### 非空的字段

非空的字段的按钮在隐藏父节点按钮旁边，默认为开启状态，只显示含有非空指标的维度。

如下图所示，仅显示含有非空指标值的维度：

![非空的字段位置](images/null_2.png)

将非空的字段取消选择，再执行查询，可以看到显示出全部的指标值，包括为空的：

![非空的字段功能](images/null_4.png)
