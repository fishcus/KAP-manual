...
spark的部署一般分为spark on YARN模式,spark on Mesos模式,和standalone模式，我们默认使用yarn模式下的spark作查询任务。该配置参数可以指定kap所使用standalone模式下的spark集群，使kap将任务提交到指定的spark-master-url。
