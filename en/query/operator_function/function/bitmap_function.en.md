...
- Parameters
  ...
  - `uuid`, the input uuid column returned by subclause. Each uuid points to a bitmap hidden from users. Function `bitmap_uuid` and `intersect_bitmap_uuid` can return uuid.
...
- Parameters
  ...
  - `t1.uuid, t2.uuid ...`, the list of input uuid. Each uuid points to a bitmap hidden from users. Function `bitmap_uuid` and `intersect_bitmap_uuid` can return uuid.
