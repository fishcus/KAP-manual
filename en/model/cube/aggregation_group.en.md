...
​            Group by pay_type, buyer_id then no cuboid can be hit, Kylin will live calculate result based on existing cuboid. 
...
Case 4: Analysts want to get different granularity of geographical dimension, with no exception, any combination can be obtained from cuboid in Diagram 8.

For instance, SELECT country, city, count(*) FROM table GROUP BY country, city, it will retrieve data from cuboid[country, province, city]
...
Case 2: If one unusual query runs

SELECT cal_dt, city, count(*) FROM table GROUP BY cal_dt, city then no cuboid can be hit, Kylin will leave calculate result based on existing cuboid. 
