...
### Pre-Conditions
* Kyligence Enterprise version should be  2.1 or later.
* Apache Kylin version should be 1.5.4.1 or later.
* KyAnalyzer does not supprt the query of *left join* currently. You may use *inner join* when creating a cube model in Kyligence Enterprise.
...
In KyAnalyzer V2.5.0 or later version, KyAnalyzer will read the Kyligence Enterprise license and does not require a separate license file.
...
3. KyAnalyzer depends on the jar package of mondrian. It needs to be downloaded and copied to KyAnalyzer's path in order to comply with open source license.
   ...
   * For KyAnalyzer with version later than 2.1.3, in Linux system, running the startup script will download and install the jar package automatically.
   * If no public network is available or in Windows system, when installing KyAnalyzer V2.5.0-2.5.1, a user can download the jar manually from [mondrian-kylin-2.0.jar](https://github.com/Kyligence/kylin-mondrian/blob/master/build/mondrian-kylin-2.0.jar), and copy it to the folder *kyanalyzer-server-{version}/tomcat/webapps/saiku/WEB-INF/lib*; For KyAnalyzer V2.5.4 or above, a user is required to download the jar from [mondrian-kylin-2.1.jar](https://github.com/Kyligence/kylin-mondrian/blob/master/build/mondrian-kylin-2.1.jar), and copy it to the folder *kyanalyzer-server-{version}/tomcat/webapps/saiku/WEB-INF/lib*.

4. Edit *kyanalyzer.properties* under *kyanalyzer-server-{version}/conf*, and set Kyligence Enterprise host and Kyligence Enterprise port. *kap.host* represents Kyligence Enterprise's IP (default value: localhost) and *kap.port* represents Kyligence Enterprise's app port (default port: 7070). And you can configure *mondrian.properties* by referring to *conf/mondrian.properties.template*. (Note: we have moved 'kap.host' and 'kap.port' to *kyanalyzer.properties* since Kyligence Enterprise V2.2, and also add *mondrian.properties* to *kyanalyzer-server/conf/*.)
...
### Compatibility of KyAnalyzer, Kyligence Enterprise and Mondrain-kylin
...
### Authentication
KyAnalyzer authenticates a user at Kyligence Enterprise side, so you can use Kyligence Enterprise user and password to login. Users with system *Admin* Role in Kyligence Enterprise are also *Admin* in KyAnalyzer.
![KyAnalyzer Login](images/analyzer_login.png)

### Sync Cube
Users may synchronize a Cube from Kyligence Enterprise. In doing so, a schema file and a datasource file for each cube from Kyligence Enterprise will be created in KyAnalyzer by default. KyAnalyzer automates the workflow, so a user doesn't need to write the schema manually.
Click **Sync Cubes From Kylin** on the left side, then the right panel will list all projects from Kyligence Enterprise.
...
Access permission to a project and cubes in KyAnalyzer are consistent with Kyligence Enterprise. For more details, please refer to [Manage Permissions](../security/acl.en.md).  

User's data access to table/row/column in KyAnalyzer would be consistent with Kyligence Enterprise. Except that in KyAnalyzer V2.5.0, a user without query permission to a table (table-level access has been removed for a table) cannot sync any Cube that using this table. For details regarding the data access control in Kyligence Enterprise, please refer to [Data Access Control](../security/data_access_control.en.md). 
...
## Kyligence Enterprise Permissions Inherit
...
KyAnalyzer inherits the permissions set in Kyligence Enterprise ,see table below.

| The range of limits of authority | Kyligence Enterprise                                    | KyAnalyzer                                                 |
| -------------------------------- | --------------------------------------- | ---------------------------------------------------------- |
