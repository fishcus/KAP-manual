...
2. 点击左侧导航栏的**建模**，再点击右侧**概览** -> **模型**标签页， 主页面将以卡片形式显示已有的数据模型。
3. 在**模型**标签页右上方，点击**列表**图标，可以切换到列表形式显示已有的数据模型。
...
在已有的模型列表中选择具体模型，点击**模型名称**，进入模型预览页面，预览模型时无法对模型进行编辑。
...
- 增量构建
  ...
  > **提示：**关于自定义增量构建的更多细节，请参考[自定义增量构建](../cube_build/customize_build.cn.md)。
...
在多维分析场景中，维度表可能会发生变化，业界称之为缓慢变化维并有着一套成熟的处理方法。对于启用了 快照存储的维度表，Kyligence Enterprise 支持定义其衍生维度的缓慢变化维处理方法，以支持不同的分析需求。更多细节请参考[缓慢变化维度](slowly_changing_dimension.cn.md).
