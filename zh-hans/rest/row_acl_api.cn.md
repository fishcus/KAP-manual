...
> 示例 Curl 请求赋予用户 ADMIN 对表 DEFAULT.KYLIN_SALES 在行 LSTG_FORMAT_NAME 取值为 'Auction' 和 'ABIN'，OPS_REGION 取值为 'BEIJING' 的行级访问控制权限，同时赋予用户 ANALYST 对表 DEFAULT.KYLIN_SALES 在行 LSTG_FORMAT_NAME 取值为 'ABIN' 的行级访问控制权限。
